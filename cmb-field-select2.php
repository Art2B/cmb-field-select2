...
/**
 * Render multi-tags input field
 */
function pw_tags_render ( $field, $value, $object_id, $object_type, $field_type_object) {
	pw_select2_enqueue();
...
add_filter( 'cmb2_render_pw_tags', 'pw_tags_render', 10, 5 );

function pw_tags_escape( $check, $meta_value ) {
	return ! empty( $meta_value ) ? implode( ',', $meta_value ) : $check;
}
add_filter( 'cmb2_types_esc_pw_tags', 'pw_tags_escape', 10, 2 );


function pw_tags_sanitize( $check, $meta_value ) {
...
add_filter( 'cmb2_sanitize_pw_tags', 'pw_tags_sanitize', 10, 2 );